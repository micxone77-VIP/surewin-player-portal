...
const NAV_ITEMS = [
  ...
  { to: '/leaderboard',   label: 'Leaderboard', icon: TrophyNavIcon },
    ...
    <nav style={{
      ...
      {NAV_ITEMS.map(({ to, label, icon: Icon }) => (
        <NavLink
          key={to}
          to={to}
          style={{ flex: 1, minWidth: 0, display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 2, padding: '2px 2px' }}
        >
          {({ isActive }) => (
            <>
...
function TrophyNavIcon({ size = 24 }) {
  return <svg width={size} height={size} viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth={1.8} strokeLinecap="round" strokeLinejoin="round"><path d="M8 4h8v5a4 4 0 01-8 0V4z" /><path d="M8 6H5a2 2 0 000 4h3M16 6h3a2 2 0 010 4h-3M12 13v4M8 21h8M9 17h6" /></svg>
}
